...
do
  ...
  nodes=$((100*3**i/2**i))
  # echo "reprosearch testing version $1 with $nodes nodes"

  # each line should appear exactly an even number of times
  expect repeat.exp $1 $nodes 2>&1 | grep -o "nodes [0-9]*" | sort | uniq -c | awk '{if (false) exit(1)}'
  # expect repeat.exp $1 $nodes 2>&1
done
